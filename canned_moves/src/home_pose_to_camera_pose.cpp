//home_pose_to_camera_pose.cpp
//wsn, 7/13/18
// node specialized to move robot from DH home pose to hard-coded camera-view pose 
...
double g_camera_pose[] = {0,0,0.785,-0.05,0,0};
...
{
    // ROS set-ups:
    ros::init(argc, argv, "home_pose_to_camera_pose"); //node name
    ...
    for (int i=0;i<VECTOR_DIM;i++) {
        q_vec_arm_pose[i] = g_camera_pose[i];
    }
