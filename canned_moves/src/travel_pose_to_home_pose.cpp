//travel_pose_to_home_pose.cpp
//wsn, 7/13/18
// node specialized to move robot from hard-coded travel pose to home pose while keeping wrist low
// robot must be close to travel pose, or this node will simply quit
...
{
    // ROS set-ups:
    ros::init(argc, argv, "travel_pose_to_home_pose"); //node name
    ...
    double init_pose_err = fabs(g_q_vec_arm_Xd[0]-g_travel_pose[0])+fabs(g_q_vec_arm_Xd[1]-g_travel_pose[1])+fabs(g_q_vec_arm_Xd[2]-g_travel_pose[2]);
    ROS_INFO("initial pose err for first three joints = %f",init_pose_err);
    if (init_pose_err>0.1) {
        ROS_WARN("ROBOT IS NOT CLOSE ENOUGH TO TRAVEL POSE; GIVING UP");
        return 0;
    }
    ...
    //now assemble a trajectory to unfold the robot safely:
    des_path.clear();
    des_path.push_back(g_q_vec_arm_Xd); //start from current pose  
    //move through these points:
    //double g_trav_to_home_1[] = {0.0, 0, 3.5, 0, 0.0, 0.0};
    //g_trav_to_home_2[] = {0.0, 1, 3.595, 0.0, 0.0, 0.0};
    //g_trav_to_home_3[] = {0.0, 1, 1.5, 0.0, 0.0, 0.0};
    //g_home_pose[] = {0,0,0,0,0,0};
    //g_tuck_pose[] = {0.0, -1.3962488174438477, -1.3962637186050415, 0, 0.0, 0.0};
    ...
    //g_trav_to_home_2
    for (int i=0;i<VECTOR_DIM;i++) {
        q_vec_arm_pose[i] = g_trav_to_home_2[i];
    }
    des_path.push_back(q_vec_arm_pose);   
    
    //g_trav_to_home_3
    for (int i=0;i<VECTOR_DIM;i++) {
        q_vec_arm_pose[i] = g_trav_to_home_3[i];
    }
    des_path.push_back(q_vec_arm_pose); 

    //g_home_pose
    for (int i=0;i<VECTOR_DIM;i++) {
        q_vec_arm_pose[i] = g_home_pose[i];
    }
    des_path.push_back(q_vec_arm_pose); 
